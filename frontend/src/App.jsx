...
export default function App() {
  ...
  const handleScan = async (text) => {
    ...
    try {
      ...
      if (verifyData.status === 'rejected') throw new Error(verifyData.reason?.message || 'Analysis failed.');
      setResults({
  ...
  if (page === 'scan') {
    return (
      <ScanPage
        uiState={uiState} results={results} error={error}
        onScan={handleScan}
        onBack={() => { setPage('home'); setUiState('idle'); setResults(null); setError(null); }}
      />
    );
  }
...
function AnimatingBadge({ verdict, safety, label, company, title, delay }) {
  const [show, setShow] = useState(false);
  const { useEffect } = require !== undefined ? { useEffect: require('react').useEffect } : {};
  return null;
}
...
function LiveBadge({ verdict, safety, label, company, title, delay }) {
  ...
  return (
    <div style={{
      ...
      transition: 'all 0.5s ease', fontFamily: FONT,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#1D4ED8', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</div>
      ...
      {show && (
        <div style={{
          ...
          padding: '3px 9px', borderRadius: '5px', background: C.bg, border: `1px solid ${C.border}`,
          animation: 'badgeFadeIn 0.4s ease',
        }}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: C.dot, display: 'block', flexShrink: 0 }}></span>
          <span style={{ fontSize: '11.5px', fontWeight: '700', color: C.text }}>{safety}% Safe</span>
...
function LandingPage({ onStartScan, onInstall }) {
  ...
  const mockJobs = [
    ...
    { verdict: 'RED',    safety: 12, label: 'High Risk', company: 'Microsofft Solutions Ltd', title: 'Remote Developer, Earn 500k Monthly', delay: 1100 },
    { verdict: 'YELLOW', safety: 62, label: 'Caution',  company: 'Didii Technologies', title: 'Frontend Product Engineer', delay: 1700 },
  ];

  const platforms = ['LinkedIn', 'Jobberman', 'Indeed Nigeria', 'MyJobMag', 'NGCareers', 'HotNigerianJobs', 'NaijaJobs', 'Graduate Jobs Nigeria'];
  ...
  return (
    <div style={{ fontFamily: FONT, background: '#F8FAFC', color: NAVY }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{
              ...
              <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: BLUE_L, display: 'block', animation: 'pulse 2s infinite' }}></span>
            ...
            <p style={{ fontSize: mobile ? '15px' : '17px', color: '#374151', lineHeight: '1.8', marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '22px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: NAVY, border: `1.5px solid ${tryH ? '#CBD5E1' : '#E2E8F0'}`,
            ...
            <div style={{ display: 'flex', gap: mobile ? '12px' : '20px', flexWrap: 'wrap' }}>
              {['No account needed', 'Works in 60 seconds', 'Free forever'].map(t => (
                <div key={t} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', color: '#1E40AF', fontWeight: '700' }}>
                  <span style={{
      ...
      <div style={{ overflow: 'hidden', borderTop: '1.5px solid #E2E8F0', borderBottom: '1.5px solid #E2E8F0', background: 'white', padding: '14px 0' }}>
        <div style={{ display: 'flex', width: 'max-content', animation: 'marquee 22s linear infinite' }}>
          {[...platforms, ...platforms].map((p, i) => (
            <span key={i} style={{
              ...
              <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: BLUE_L, display: 'block', flexShrink: 0 }}></span>
              {p}
            </span>
          ))}
        </div>
      ...
      <section id="how-it-works" style={{ padding: '70px 6%', maxWidth: '1160px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '52px' }}>
          <h2 style={{ fontSize: mobile ? '28px' : '36px', fontWeight: '900', color: NAVY, marginBottom: '14px', letterSpacing: '-0.5px', fontFamily: FONT }}>
            How it looks on your feed
          </h2>
          <p style={{ fontSize: '16px', color: '#374151', maxWidth: '520px', margin: '0 auto', lineHeight: '1.75', fontFamily: FONT }}>
        ...
        <div style={{
          ...
          <div style={{ background: 'white', borderRadius: '14px', overflow: 'hidden', border: '1.5px solid #E2E8F0', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
            <div style={{ background: '#0A66C2', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ background: 'white', borderRadius: '4px', padding: '3px 7px', fontWeight: '900', fontSize: '13px', color: '#0A66C2' }}>in</div>
              <span style={{ color: 'white', fontSize: '13px', fontWeight: '600' }}>Job listings on any platform</span>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[
              { dot: GREEN, bg: '#ECFDF5', border: '#6EE7B7', text: '#065F46', title: 'Safe', desc: 'Established company with clean domain infrastructure and no threat signals. Apply with confidence.' },
              { dot: ORANGE, bg: '#FFF7ED', border: '#FED7AA', text: '#9A3412', title: 'Caution', desc: 'New or unverified company. No active scam signals found but limited history. Research before applying.' },
              { dot: RED, bg: '#FEF2F2', border: '#FECACA', text: '#991B1B', title: 'High Risk', desc: 'Active threat signals detected. This matches known scam patterns. Do not share any personal information.' },
            ].map(item => {
              const [h, hh] = useHover();
              return (
                <div key={item.title} {...hh} style={{
                  display: 'flex', gap: '14px', padding: '18px',
                  background: 'white', borderRadius: '12px',
                  border: '1.5px solid #E2E8F0',
                  boxShadow: h ? '0 4px 16px rgba(0,0,0,0.08)' : 'none',
                  transition: 'all 0.2s', transform: h ? 'translateY(-2px)' : 'translateY(0)',
                  ...
                  <div style={{
                    ...
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: item.dot, display: 'block' }}></span>
                  </div>
                  <div>
                    <div style={{ fontWeight: '800', fontSize: '15px', color: item.text, marginBottom: '4px' }}>{item.title}</div>
                    <p style={{ fontSize: '13.5px', color: '#374151', margin: 0, lineHeight: '1.65' }}>{item.desc}</p>
                  </div>
                </div>
              );
            })}
      ...
      <section style={{ background: NAVY, padding: mobile ? '60px 6%' : '80px 6%', textAlign: 'center' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: mobile ? '28px' : '38px', fontWeight: '900', color: 'white', marginBottom: '18px', letterSpacing: '-0.6px', lineHeight: '1.15', fontFamily: FONT }}>
          ...
          <CTA_Button onClick={onInstall} />
          <div style={{ marginTop: '20px', display: 'flex', gap: mobile ? '12px' : '24px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {['No account required', 'Works in 60 seconds', 'Free forever for job seekers'].map(t => (
              <span key={t} style={{ fontSize: '13.5px', color: '#64748B', fontWeight: '500', fontFamily: FONT }}>{t}</span>
            ))}
          </div>
        </div>
      </section>
